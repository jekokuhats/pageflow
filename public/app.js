...
async function summarizeWithAI(text) {
    ...
    // Clear previous bot messages if any (optional, or keep history)
    // chatBody.innerHTML = ''; 

    addChatMessage('user', `Summarize this: "${text.substring(0, 50)}..."`);
    ...
    try {
        const response = await fetch(
            `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${GEMINI_API_KEY}`,
            {
                ...
                body: JSON.stringify({
                    contents: [{ parts: [{ text: "Context: This is a PDF reader. User selected a text. Task: Summarize this text concisely in 3-4 bullet points. Text: " + text }] }]
                })
            }
        );
...
function streamBotMessage(fullText) {
    const chatBody = document.getElementById('ai-chat-body');
    const msgDiv = document.createElement('div');
    msgDiv.className = 'ai-message bot-message';
    ...
    const interval = setInterval(() => {
        if (i < fullText.length) {
            // Simple markdown-ish replacement for newlines
    ...
    }, 15); // Adjust speed for "streaming" feel
}
...
function handleTextSelection(e) {
    // Ignore clicks inside the highlight picker
    if (e.target.closest('#highlight-picker')) {
        return;
    }

    const sel = window.getSelection();
    if (sel.rangeCount > 0 && sel.toString().trim().length > 0 && e.target.closest('#pdf-wrapper')) {
        selectedText = sel.toString().trim();
        const range = sel.getRangeAt(0);
        const rects = range.getClientRects();

        // Calculate coords relative to canvas/viewport BEFORE scaling
        const wrapper = document.getElementById('pdf-wrapper');
        ...
        // Use either mouse coordinates or touch coordinates for popup placement
        const clientY = e.clientY || (e.changedTouches ? e.changedTouches[0].clientY : 0);
        const clientX = e.clientX || (e.changedTouches ? e.changedTouches[0].clientX : 0);

        picker.style.top = (clientY + 20) + 'px';
